...
# loop test, game-flow integration test, then the rendered smoke test.
...
echo "===== [1/7] physics stress test (250 rigid bodies, Jolt) ====="
"$GODOT" --headless --audio-driver Dummy --path . res://tests/StressTest.tscn

echo
echo "===== [2/7] destruction-core test (structure -> shatter -> debris) ====="
"$GODOT" --headless --audio-driver Dummy --path . res://tests/DestructionTest.tscn

echo
echo "===== [3/7] audio wiring test (pew/crash/impact trigger mapping) ====="
"$GODOT" --headless --audio-driver Dummy --path . res://tests/AudioTest.tscn

echo
echo "===== [4/7] audio spectral analysis (low-end crashes, crisp pews) ====="
python3 tools/analyze_audio.py

echo
echo "===== [5/7] weapon + damage loop test (fire -> shatter, debris -> damage, death -> respawn) ====="
"$GODOT" --headless --audio-driver Dummy --path . res://tests/WeaponDamageTest.tscn

echo
echo "===== [6/7] game-flow integration test (HUD + death -> game-over -> auto-respawn) ====="
"$GODOT" --headless --audio-driver Dummy --path . res://tests/GameFlowTest.tscn

echo
echo "===== [7/7] rendered smoke test (first-person camera + movement) ====="
./smoke_test.sh
